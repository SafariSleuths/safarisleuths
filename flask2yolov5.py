...
def cropped_imgs_s3_individuals(df):
    ...
    for idx, row in df.iterrows():
        ...
        if species == 'Crocuta_crocuta':
            ...
            s3.meta.client.copy(copy_source, app.config['S3_BUCKET'], f'/hyena.coco/processed/train/{animal_id}')
        elif species == 'Panthera_pardus':
            s3.meta.client.copy(copy_source, app.config['S3_BUCKET'], f'/leopard.coco/processed/train/{animal_id}')
        elif species == 'Giraffa_tippelskirchi':
            s3.meta.client.copy(copy_source, app.config['S3_BUCKET'],
                                f'/great_zebra_giraffe/individual_recognition/train/{animal_id}')
        # Copy the image to the all individuals folder to enable retraining of the embeddings
        s3.meta.client.copy(copy_source, app.config['S3_BUCKET'], '/all_animal_recognition/train/animal_id')
